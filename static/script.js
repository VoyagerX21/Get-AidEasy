...
function filterCategories(query) {
    ...
    return courses.filter(category =>
...
function renderDropdown(items) {
    ...
    dropdown.innerHTML = items
        .filter(item => !item.Modules.Courses.toLowerCase().includes("course series"))
        .map((item) => `
...
function selectItem(item) {
    ...
    loader.innerHTML = '<div style="border: 4px solid #f3f3f3; border-top: 4px solid #3498db; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite;"></div><style>@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); }}</style>';
    ...
    switch (e.key) {
        ...
        case 'Enter':
            ...
            if (currentHighlight >= 0 && items[currentHighlight]) {
                ...
                const selectedItem = filteredCategories.find(i => String(i.FIELD1) === field1) || items.find(i => String(i.FIELD1) === field1);
                if (selectedItem) {
                    selectItem(selectedItem);
                } else {
                    console.error('Item with FIELD1 not found:', field1);
